...
public class DownloadXml {
    public static String downloadXml(int id) throws UnsupportedEncodingException {
        List<PageInspectTable> al = new ReadInspectTableTest().getAll(id);
        List<PageInspectTable> bl = new ReadInspectTableTest().getTag(id);
        List<PageInspectTable> valuelist=new ReadInspectTableTest().getTvaluesByTableId(id);
         String result=null;
        boolean b = false;
        ...
        while (list.hasNext()) {
            ...
            for (int i = 0; i < cl.size(); i++) {
                ...
                Iterator value=valuelist.iterator();
                while(value.hasNext()){
                    PageInspectTable val=(PageInspectTable)value.next();
                    tableEle.addElement("value").addAttribute("name", val.getTvalue());
                }
            }
        }
        try {
